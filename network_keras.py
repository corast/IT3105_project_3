...
import os
class Model():
    def __init__(self,model, optimizer, loss, name, filepath=None, input_type=1):
        if(type(model) == list):
    ...
    def train(self, datamanager:Datamanager.Datamanager, epochs, batch_size):
        X,Y = datamanager.return_keras(self.input_type)# Return all data in CSV file. 
...
#    
def CNN_50_25(name="CNN-50-25",dim=5):
    return Model(model=[
